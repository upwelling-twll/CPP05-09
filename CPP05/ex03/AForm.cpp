...
{
	std::cout << "Default execution by " << executor.getName() << std::endl;
	return (true);
}
...
AForm::AForm(std::string const &target) : _gradeToSign(150), _gradeToExecute(150), _target(target)
{
	std::cout << "AForm constructor is called with target" << std::endl;
}
